    ...
    **Ответь ТОЛЬКО валидным JSON** без дополнительного текста:
    ...
    def evaluate(
        ...
        prompt = self.prompt_template.format(
            context=self._escape_braces(context_str),
            question=self._escape_braces(question),
            answer=self._escape_braces(answer),
            ground_truth=self._escape_braces(ground_truth)
        )

        try:
            response = self.llm.invoke(prompt)

            print(f'RAGEvaluator response LLM {response}')
